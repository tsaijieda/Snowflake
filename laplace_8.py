...
def grow_boundary_proportional_to_gradient(u, hex_mask, gx, gy, nx, ny, growth_factor, cutoff):
    """
...
def smooth_mask(mask, sigma=1):
    ...
    smoothed_mask = smoothed_mask > 0.5  # Threshold back to binary
...
for i in range(10):
    ...
    expanded_mask = grow_boundary_proportional_to_gradient(updated_solution, updated_mask, gx, gy, nx, ny, 500, cutoff)
    filled_mask = fill_concave_hull(expanded_mask, nx, ny, alpha=0.2)

    # Apply smoothing to the filled mask
    smoothed_mask = smooth_mask(filled_mask, sigma=0.5)
