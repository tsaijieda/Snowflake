...
# Solve the Laplace equation for the initial state
updated_solution, updated_mask = solve_laplace_outside_hexagonal(nx, ny, tol)

for i in range(10):
    gy, gx = np.gradient(-updated_solution)
    expanded_mask = grow_boundary_proportional_to_gradient(updated_solution, updated_mask, gx, gy, nx, ny, 500)

    # Compute concave hull without filling
    y, x = np.where(expanded_mask)
    points = np.column_stack((x, y))
    alpha_shape = alphashape.alphashape(points, alpha=0.2)

    plt.figure(figsize=(8, 6))
    plt.imshow(expanded_mask, cmap='gray', origin='lower', extent=[0, 1, 0, 1])
    if alpha_shape.geom_type == 'Polygon':
        x_hull, y_hull = alpha_shape.exterior.xy
        plt.plot(np.array(x_hull) / nx, np.array(y_hull) / ny, color='red', linewidth=2, label='Concave Hull (Unfilled)')
    plt.title('Expanded Mask with Concave Hull (Unfilled)')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.legend()
    ...
    plt.colorbar(label='Boundary (1: Inside, 0: Outside)')
    plt.title('Expanded Mask with Concave Hull (Filled)')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.show()
    ...
    plt.xlabel('x')
    plt.ylabel('y')
    plt.contour(np.arange(nx) / nx, np.arange(ny) / ny, filled_mask, levels=[0.5], colors='cyan', linewidths=2, linestyles='--')
    plt.show()
